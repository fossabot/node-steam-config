...
const pj = JSON.parse(require('fs').readFileSync(path.join(__dirname, 'package.json')))
...
(async () => {
  const options = cli.parse({
    ...
    source: ['s', 'The directory to search for source files to extract docs from.', 'path', path.join(__dirname, 'src')],
    ...
    intermediary: ['i', 'Intermediary output file (JSON).', 'path', path.join(__dirname, 'doccomments.json')],
    out: ['o', 'The output file', 'path', path.join(__dirname, 'API.md')],
